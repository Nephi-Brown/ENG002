...
def turn_left(count_target, base_speed=60):
...
def turn_right(count_target, base_speed=60):
    global encoder_countl, encoder_countr
    encoder_countl = 0
    encoder_countr = 0
    while abs(encoder_countl) < count_target and abs(encoder_countr) < count_target:
...
def right_wall_following(right_mm, front_mm):
    ...
    if mode == "forward":
        ...
        if right_mm > gap + 20:   # margin above gap
            ...
            stop()
            time.sleep(1)
            turn_right(right_turn_counts)
            time.sleep(1)
            encoder_countr = 0
            encoder_countl = 0
            while encoder_countr < gap_roll_counts_2 and encoder_countl < gap_roll_counts_2: #this may thro logic 
                correct_motors_custom(50, 50)
                time.sleep_ms(1)
        ...
        elif front_mm < safe_front - 10 and right_mm < gap + 20:# margin below safe_front
            mode = "turning_left"
            pid_active = False
            stop()
            time.sleep(2)
...
left_turn_counts = 30
right_turn_counts = 25
gap_roll_counts = 45
gap_roll_counts_2 = 90
gap = 40
safe_front = 60
pid_active = True
