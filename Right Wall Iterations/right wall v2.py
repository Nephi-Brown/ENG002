...
FRONT_TURN_DIST   = 30
FRONT_REPEAT_DIST = 40

# - RIGHT OPENING BEHAVIOUR -
IR_OPEN_FORWARD_MM       = 80
IR_OPEN_EXTRA_FORWARD_MM = 50
...
def PID_Drive_Step(left_mm, right_mm):
    ...
    # ============================================================
    if left_ir.value() == 1 and left_mm > 0 and right_mm > 0:

        side_error = left_mm - right_mm
        steering = CENTER_GAIN * side_error
        steering = _clamp(steering, -STEER_MAX, STEER_MAX)

        L_corrected = L_pwm - steering
        R_corrected = R_pwm + steering

        L_corrected = _clamp(L_corrected, -MAX_DRIVE_PWM, MAX_DRIVE_PWM)
        R_corrected = _clamp(R_corrected, -MAX_DRIVE_PWM, MAX_DRIVE_PWM)
