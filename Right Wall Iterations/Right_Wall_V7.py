...
# ===== NEW: Reset PID + detection state after big maneuvers =====

def Reset_PID_and_Detection():
    global previous_left, previous_right, last_L, last_R, last_time

    # Forget old side-wall history so we don't detect fake "corners"
    previous_left = None
    previous_right = None
    ...
    # Reset last encoder/time used by PID_Drive so next speed calc starts fresh
    last_L = Left_Encoder_Count
    last_R = Right_Encoder_Count
    last_time = time.ticks_ms()
...
def Wall_Turn_Right():
    ...
    Drive_Forward_mm(260)
    time.sleep(0.5)
    
    Motor_Stop()
    time.sleep(0.5)

    # NEW: reset after a big wall-following maneuver
    Reset_PID_and_Detection()
...
def Handle_Front_Wall(front_mm):
    ...
    # NEW: after a front-induced turn, also reset detection + PID/encoder timing
    Reset_PID_and_Detection()
    time.sleep(0.5)
    return
...
def Left_Straight_Turn():
    Left_Motor_Set(-10000)
    Right_Motor_Set(10000)
    time.sleep(0.01)
...
try:
    while True:
        ...
        left_mm = _clamp(left_mm, 20, 180)
        right_mm = _clamp(right_mm, 20, 180)
        front_mm = _clamp(front_mm, 25, 1200)
        
        right_triggered, previous_right = Detect_Wall(right_mm, previous_right)
        left_triggered, previous_left   = Detect_Wall(left_mm, previous_left)
        ...
        # Keep your original reads (even if not strictly needed)
        new_left_mm = Read_Left_mm()
        new_right_mm = Read_Right_mm()
        
        # NEW centering logic:
        # - Still center using both sides.
        # - But if a left-side gap/corner is detected, don't let the
        #   sudden big left distance pull us toward the gap.
        #   Instead, pretend left distance ≈ right distance for this step.
        center_left_mm = left_mm
        center_right_mm = right_mm

        if left_triggered:
            center_left_mm = center_right_mm

        PID_Drive(center_left_mm, center_right_mm)
        time.sleep(0.01)
