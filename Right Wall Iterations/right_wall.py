...
# - TURNING -
TURN_TIME_90 = 0.25   # s, tune for your robot
TURN_SPEED   = 15000
...
FRONT_TURN_DIST   = 70    # 7cm -> initial left 90
FRONT_REPEAT_DIST = 100   # 10cm -> repeat 90 if still too close

# - RIGHT-OPENING BEHAVIOUR (mm) -
IR_OPEN_FORWARD_MM      = 80    # 8cm forward before turning right
...
# ----- IR SETUP -----
#  Initialise only RIGHT IR (left IR intentionally not used)
...
# ---------- PID DRIVE STEP (same logic as original main.py) ----------
...
def PID_Drive_Step(left_mm, right_mm):
    """One PID + centering update, using the same logic as original main.py."""
    ...
    # - WALL CENTERING (same as original) -
...
# ---------- RIGHT WALL-OPEN HANDLING (IR = 0) ----------

def Handle_Right_Open():
    """
    Right IR = 0 (no wall):
      - drive forward 8cm using PID + centering
      - 90° right turn on the spot
      - record both side ToFs
      - drive forward; if either side ToF falls below half recorded, drive
        only 5cm more before returning to normal algorithm.
    """
    print("Right IR=0 → forward 8cm, right 90, then monitor side ToFs")

    # 1) 8 cm forward
    Drive_Forward_mm(IR_OPEN_FORWARD_MM)

    # 2) 90° right on the spot
    Turn_Right_90()
    time.sleep(0.1)

    # 3) Record side ToF values after the turn
    base_left  = Read_Left_mm()
    base_right = Read_Right_mm()
    print("Side base distances L/R:", base_left, base_right)

    # 4) Drive forward, looking for sudden side drop
    while True:
        front_mm = Read_Front_mm()
        left_mm  = Read_Left_mm()
        right_mm = Read_Right_mm()

        # Avoid smashing into a wall while in this mode
        if 0 < front_mm <= FRONT_TURN_DIST:
            Stop_Motors()
            Handle_Front_Wall(front_mm)
            return

        # Check for sudden side drop
        drop = False
        if base_left > 0 and left_mm > 0 and left_mm < base_left * SIDE_DROP_FACTOR:
            drop = True
        if base_right > 0 and right_mm > 0 and right_mm < base_right * SIDE_DROP_FACTOR:
            drop = True

        if drop:
            print("Side drop detected, extra 5cm then normal mode")
...
try:
    while True:
        ...
        right_ir_val = right_ir.value()  # 1 = wall detected, 0 = no wall
        ...
        # 2) RIGHT-OPENING HANDLING (right-hand rule; IR 0 means opening)
        if right_ir_val == 0:
            Stop_Motors()
            Handle_Right_Open()
            continue
