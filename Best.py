...
def Turn_45_Right():
    ...
    TURN_MM = ( (math.pi * 95.0) / (360 / angle) )
    TARGET_COUNTS = int(TURN_MM * COUNTS_PER_MM * TURN_SCALE_RIGHT)
...
def Drive_Forward_mm(dist_mm):
    ...
    while True:
        ...
        Right_PWM = int(round(Right_PWM * 1.025))
...
def Check_45_Turn():
    ...
    if front_mm <= FRONT_CHECK and left_mm >= SIDE_CHECK and right_mm >= SIDE_CHECK:
        if right_mm >= left_mm:
            Turn_45_Right()
            time.sleep(0.5)
        else:
...
def Detect_Wall_Increase(current, previous):
    if previous is None:
        return False, current
    ...
    wall_far = current >= MIN_SIDE_DIST_FAR

    triggered = corner and wall_far

    return triggered, current

# ================== SMALL "STRAIGHTEN" TURNS ==================

def After_Turn_Straighten():
    left_mm = Read_Left_mm()
    right_mm = Read_Right_mm()
    
    delta = round ( ( left_mm / right_mm ) , 2 )
...
try:
    ...
    while True:
        ...
        right_far_triggered, previous_right = Detect_Wall_Increase(right_mm, previous_right)
        
        left_far_triggered, previous_left = Detect_Wall_Increase(left_mm, previous_left)
        ...
        if right_far_triggered:
            Right_Straight_Turn_Small()
            Reset_State()
            continue
            
        if left_far_triggered:
            Left_Straight_Turn_Small()
            Reset_State()
            continue
        
        if raw_left_mm > 0 and raw_left_mm <= 8:
            Right_Straight_Turn()
            Drive_Forward_mm(2)
            Reset_State()
            continue
            
        if raw_right_mm > 0 and raw_right_mm <= 8:
            Left_Straight_Turn()
            Drive_Forward_mm(2)
            Reset_State()
            continue
