...
counts = 15
gap_roll_counts = 20
right_gap_threshold = 150 # so robot knows when to turn right 
safe_front = 60 # so robot knows when to turn left 
pid_active = True
...
try:
    while True:
        ...
        if right_mm >= right_gap_threshold: # if there is a right opening 
            print("Turning right. Right sensor: ", right_mm)
            pid_active = False 
            stop() # stop the motors
            time.sleep(1)
            encoder_countr = 0 # reset encoder counts 
            encoder_countl = 0
            while encoder_countr < gap_roll_counts and encoder_countl < gap_roll_counts:
                correct_motors_custom(50, 50)
                time.sleep_ms(1) # drive forward into the gap
            stop() # stop the motors
            time.sleep(1)
            turn_right(counts) # turn right 
            wait(50)
            pid_active = True
            
        if 0 < front_mm <= safe_front:
            print("Turning left. Front sensor: ", front_mm)
            pid_active = False
            stop()
            time.sleep(1)
            turn_left(counts)
