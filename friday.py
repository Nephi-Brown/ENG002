...
gap_roll_counts = 40
gap_roll_countss = 40
right_open_threshold = 50
front_wall_threshold = 45
pid_active = True
...
try:
    while True:
        ...
        if right_mm > right_open_threshold:
            ...
            #stop()
            #time.sleep(1)
            if front_mm > front_wall_threshold:
                encoder_countr = 0
                encoder_countl = 0
                while (encoder_countr < gap_roll_counts or encoder_countl < gap_roll_counts):
            ...
            pid_active = True
            continue
        elif front_mm < front_wall_threshold:  # margin below safe_front
            pid_active = False
            stop()
            time.sleep(1)
            while front_mm < front_wall_threshold:
