...
def correct_motors(correction_fix):
    ...
    fix_left = max(min(fix_left, 100), -100)
    fix_right = max(min(fix_right, 100), -100)
...
def right_wall_following(right_mm, front_mm):
    global pid_active, encoder_countr, encoder_countl, mode, just_turned
    if mode == "forward":
        if right_mm > gap + 20:   # margin above gap
            ...
            if front_mm > safe_front - 10:
                ...
                encoder_countr = 0
                encoder_countl = 0
                while encoder_countr < gap_roll_countss and encoder_countl < gap_roll_countss:
            ...
            wait(1000)
            front_mm = tof_forward.read_range() if tof_forward else -1
            if front_mm >= safe_front + 20:
                while encoder_countr < 10:
                    correct_motors_custom(48, 50)
                    time.sleep_ms(1)
            pid_active = True
            mode = "forward"
