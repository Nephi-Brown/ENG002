...
# ignoring left openings/ managing centering after turns stuff
last_left_mm = None 
side_wall_centering_active = True
pending_right_wall = False
...
def Turn_Left_90():
    global Left_Encoder_Count, Right_Encoder_Count, last_left_mm, side_wall_centering_active, pending_right_wall
    ...
    last_left_mm = None # wall memory not valid after turning
    # side wall centering stopped until right wall seen again
    side_wall_centering_active = False 
    pending_right_wall = True 
...
def PID_Drive(left_mm, right_mm):
    global last_L, last_R, last_time, side_wall_centering_active
    ...
    # Side wall centering
    if side_wall_centering_active and 0 < left_mm < 150 and 0 < right_mm < 150:
...
def Handle_Front_Wall(front_mm):
    ...
    if RIGHT_OPEN:
        print("Turning RIGHT from front wall")
        Turn_Right_90()
        time.sleep(0.5)
    elif LEFT_OPEN:
        print("Turning LEFt from front wall")
        Turn_Left_90()
        time.sleep(0.5)
    else:
        print(" Dead end → Left Turn ")
        Turn_Left_90()
        time.sleep(0.5)
        Turn_Left_90()
        time.sleep(0.5)
...
LEFT_WALL_THRESHOLD = 60  # sensor reading required to believe there is a wall

def ignore_left_openings(left_mm):
    global last_left_mm, LEFT_WALL_THRESHOLD
    
    # first run
    if last_left_mm is None and left_mm > 0:
        last_left_mm = left_mm
        return left_mm
    
    # if real wall seen update memory and use that value
    if 0 < left_mm < LEFT_WALL_THRESHOLD:
        last_left_mm = left_mm
        return left_mm
    
    # if opening use stored value 
    if last_left_mm is not None:
        return last_left_mm
    else:
        return left_mm

# ===== RIGHT WALL REACQUISITION AFTER TURN =====

def reacquire_right_wall(right_mm):
    global pending_right_wall, side_wall_centering_active
    if not pending_right_wall:
        return
...
RIGHT_OPEN_THRESHOLD = 90
FRONT_CLEAR = MIN_FRONT_DIST + 30 
...
try:
    while True:
        ...
        # ignore left openings for PID
        left_mm = ignore_left_openings(left_mm)
        
        # after a turn, wait until the right wall comes back before using side PID
        reacquire_right_wall(right_mm)
        
        if right_mm >= RIGHT_OPEN_THRESHOLD 
            Motor_Stop()
            Wall_Turn_Right()
            continue
        
        # front wall behaviour: right-hand rule at junctions / dead ends
        if 0 < front_mm <= MIN_FRONT_DIST:
            Motor_Stop()
            Handle_Front_Wall(front_mm)
            continue

        # otherwise just drive straight with PID + wall centering (when enabled)
