...
TURN_SCALE_LEFT = 1.75

TURN_SCALE_RIGHT = 1.7
...
def Turn_Left_90(MULTIPLIER):
    global Left_Encoder_Count, Right_Encoder_Count, turning
    turning = True
    ...
    TARGET_COUNTS = int(TURN_MM * COUNTS_PER_MM * TURN_SCALE_LEFT * MULTIPLIER)
...
def Turn_Right_90(MULTIPLIER):
    ...
    TURN_MM = (math.pi * 95.0) / 4.0
    TARGET_COUNTS = int(TURN_MM * COUNTS_PER_MM * TURN_SCALE_RIGHT * MULTIPLIER)
...
def PID_Drive(left_mm, right_mm):
    ...
    if left_valid and right_valid:
        if 20 < left_mm < 180 and 20 < right_mm < 180:
            if abs(left_mm - right_mm) > 80:
                return
            
            side_error = right_mm - left_mm
            steering = CENTER_GAIN * side_error

            steering = _clamp(steering, -MAX_STEER, MAX_STEER)

            final_left  = Left_PWM  - steering
            final_right = Right_PWM + steering

            final_left  = _clamp(final_left,  -MAX_PWM, MAX_PWM)
            final_right = _clamp(final_right, -MAX_PWM, MAX_PWM)

            Left_Motor_Set(final_left)
            Right_Motor_Set(final_right)
            return
...
def Handle_Front_Wall(front_mm):
    global previous_left, previous_right, turning
    turning = True
...
try:
    while True:
        ...
        if not (30 <= left_mm <= 200):  left_valid = False
        else:                           left_valid = True

        if not (30 <= right_mm <= 200): right_valid = False
        else:                           right_valid = True
