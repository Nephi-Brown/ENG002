...
def Compute_Steering(left_mm, right_mm):
    left_valid  = 20 < left_mm < 180
    right_valid = 20 < right_mm < 180

    if left_valid and right_valid:
        side_error = left_mm - right_mm 
        steering = CENTER_GAIN * side_error
        return _clamp(steering, -MAX_STEER, MAX_STEER)

    if left_valid and not right_valid:
        side_error = left_mm - TARGET_WALL_DIST
        steering = CENTER_GAIN * side_error
        return _clamp(steering, -MAX_STEER, MAX_STEER)

    if right_valid and not left_valid:
        side_error = TARGET_WALL_DIST - right_mm
        steering = CENTER_GAIN * side_error
        return _clamp(steering, -MAX_STEER, MAX_STEER)
...
def PID_Drive(left_mm, right_mm):
    ...
    measured_rps_L = (Left_Counts / COUNTS_PER_REV) / dt
    measured_rps_R = (Right_Counts / COUNTS_PER_REV) / dt
    ...
    steering = Compute_Steering(left_mm, right_mm)
...
try:
    while True:
        ...
        PID_Drive(new_left_mm, new_right_mm)
        time.sleep(0.01)
