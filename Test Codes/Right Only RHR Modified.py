...
CENTER_GAIN = 30          # steering strength
MAX_STEER = 5000

RIGHT_WALL_TARGET = 30    # mm, desired distance from the right wall
...
MIN_FRONT_DIST = 65      # mm: if front distance below this, handle front wall
MIN_INCREASE   = 30      # mm: sudden increase in side distance → gap / corner
WALL_THRESHOLD = 70      # mm: above this = "open" or gap
...
def Right_Motor_Set(speed):
    speed = speed * 1.04   # small compensation for right motor
    speed = int(speed)
    speed = _clamp(speed, -MAX_PWM, MAX_PWM)
...
def Drive_Forward_Sensor_mm(dist_mm, front_mm)
    global Left_Encoder_Count, Right_Encoder_Count, MIN_FRONT_DIST
    ...
    front_calculated = front_mm - dist_mm
...
try:
    ...
    while True:
        ...
        left_mm  = _clamp(left_mm, 20, 180)    # only used for safety checks
        front_mm = _clamp(front_mm, 25, 1200)
        
        # Right-wall gap / corner detection only
        ...
        if right_triggered:
            Motor_Stop()
            Reset_State()    # clear PID integrals & speed history
            Drive_Forward_mm(50)
            time.sleep(1.5)
            Wall_Turn_Right()
            continue
        ...
        new_right_mm = _clamp(new_right_mm, 20, 180)
        
        
        #Test code for new drive function using front sensor - x (100) then stop
        Drive_Forward_Sensor_mm(100, front_mm)
