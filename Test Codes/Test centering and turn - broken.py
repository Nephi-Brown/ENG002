...
# - TURNING -
TURN_TIME_90 = 0.375   # s, tune for your robot
TURN_SPEED   = 10000
...
FRONT_TURN_DIST   = 60    # 7cm -> initial left 90
...
def Handle_Left_Open():
    ...
    Left IR = 0 (no wall):
      ...
      - drive forward; if either side ToF falls below half recorded, drive
        only 5cm more before returning to normal algorithm.
    """
    print("Left IR=0 → forward 8cm, right 90, then monitor side ToFs")
...
try:
    while True:
        ...
        if right_ir_val == 0:
            Stop_Motors()
        elif 0 < front_mm <= FRONT_TURN_DIST:
            Handle_Front_Wall(front_mm)
            Handle_Right_Open()
            continue
